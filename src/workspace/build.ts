...
export async function runPlan(
	...
	for (const step of steps) {
		...
		if (code !== 0) {
			...
			// them nothing: `bun run hydrate` needs a published release payload and the route that
			// needs none is `build:local`. It rides the error's `next` rather than stdout, because
			// `build --json` promises stdout parses and two prose lines in front of the object is
			// the one failure a new user is most likely to hit
			throw new DranglerError(
				'build-step',
				step.id === 'hydrate'
					? `hydrate failed: \`${rendered}\` exited ${code}. It needs a published release payload and this checkout found none`
					: `${step.id} failed: \`${rendered}\` exited ${code}`,
				{
					retryable: true,
					next:
						step.id === 'hydrate'
							? `cd ${workspace} && bun run build:local`
							: `cd ${workspace} && ${rendered}`
				}
			);
		}
